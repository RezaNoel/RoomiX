...
const RoomForm: React.FC<RoomFormProps> = ({ onRoomUpdated }) => {
  const [roomData, setRoomData] = useState({
    ...
    meal_plan: "breakfast",
    is_available: true,
    image: null as File | null, // فیلد تصویر به صورت فایل
  });
  ...
  const validateForm = () => {
    const requiredFields = ["number", "name_fa", "room_type", "base_price"];
    for (const field of requiredFields) {
      if (!roomData[field as keyof typeof roomData]) {
        setError(`فیلد ${field} ضروری است.`);
        return false;
      }
    }
    setError(null);
  ...
  return (
    <form className={styles.roomForm} onSubmit={handleSubmit}>
      ...
      <input name="number" placeholder="شماره اتاق" onChange={handleChange} value={roomData.number} />
      <input name="name_fa" placeholder="نام" onChange={handleChange} value={roomData.name_fa} />
      ...
      <input name="base_price" placeholder="قیمت پایه" onChange={handleChange} value={roomData.base_price} />
      <input name="bed_count" placeholder="تعداد تخت" onChange={handleChange} value={roomData.bed_count} />
      <input name="floor" placeholder="طبقه" onChange={handleChange} value={roomData.floor} />
